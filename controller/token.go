package controller
        ...
        if token.StartOnFirstUse {
            if cleanToken.FirstUsedTime == 0 {
                // 未使用：允许 -1（延后至首次使用再计算过期时间）
                cleanToken.ExpiredTime = token.ExpiredTime
            } else {
                // 已使用：若传入 -1 且有有效持续时长，则按 首次使用时间 + 持续时长 计算；否则沿用传入值
...
func AddToken(c *gin.Context) {
    ...
    if len(token.Name) > 30 {
        c.JSON(http.StatusOK, gin.H{"success": false, "message": "令牌名称过长"})
        return
    }
...
func UpdateToken(c *gin.Context) {
    ...
    c.JSON(http.StatusOK, gin.H{"success": true, "message": "", "data": buildTokenDTO(cleanToken)})
}
